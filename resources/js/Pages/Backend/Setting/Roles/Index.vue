...
const deleteItemConfirm = () => {
    if (selectedItem.value) {
        router.visit(route("roles.destroy", selectedItem.value.id), { method: 'delete' });
    }
        ...
        <v-container class="elevation-4 mt-16 mb-16">
            <v-row class="mb-10 align-center">
                ...
                <v-col cols="12" md="6" class="d-flex justify-end">
                    ...
                    <v-btn
                        v-if="$page.props.user.roles.includes('Superadmin') || $page.props.user.permissions.includes('Create Role')"
                        color="primary"
                        class="ml-2"
                        @click="create"
                    >
                ...
                <v-col cols="12">
                    ...
                    >
                        <template v-slot:item.actions="{ item }">
                            <v-icon
                                v-if="$page.props.user.roles.includes('Superadmin') || $page.props.user.permissions.includes('Edit Role')"
                                class="me-2"
                                size="small"
                                @click="editItem(item)"
                            >
                            ...
                            <v-icon
                                v-if="$page.props.user.roles.includes('Superadmin') || $page.props.user.permissions.includes('Delete Role')"
                                size="small"
